...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsampling
        x0_h, x0_w = x[0].size(2), x[0].size(3) #高和宽 x[0] 32 128 128
        x1 = F.interpolate(x[1], size=(x0_h, x0_w), mode='bilinear', align_corners=True) # 64 128 128
        x2 = F.interpolate(x[2], size=(x0_h, x0_w), mode='bilinear', align_corners=True)#128 128 128
        x3 = F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=True)#256 128 128
        # print("x1",x[1].size()) #64 64 64
        # print("x2",x[2].size()) #128 32 32 
        # print("x3",x[3].size()) #256 16 16
        # x = torch.cat([x[0], x1, x2, x3], 1)#8 256 128 128
        

        # x = self.last_layer(x)
        
        return s1,x[0],x[1],x[2],x[3]
